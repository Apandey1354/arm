...
import { Upload as UploadIcon, X, Loader2, AlertCircle, FileImage, Volume2 } from "lucide-react"
...
export default function UploadPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      formDataToSend.append("cityLastSeen", data.cityLastSeen)
      formDataToSend.append("dateLastSeen", data.dateLastSeen)
      formDataToSend.append("contactPhone", data.contactPhone)
      formDataToSend.append("nearbyPoliceStation", data.nearbyPoliceStation)
      if (data.additionalDescription) {
        formDataToSend.append("additionalDescription", data.additionalDescription)
      }
      ...
      let description = `Your information with ${result.imagesProcessed} image(s) has been submitted. ${result.facesDetected} face(s) detected.`
      if (result.audioProcessed) {
        description += result.voiceDetected ? " Voice embedding extracted." : " Audio processed (no voice detected)."
      }
      description += " If any lead is found, we will contact the nearby police station you provided for immediate action."
          ...
          <Card className="shadow-2xl border-2 border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 hover:shadow-primary/10 transition-shadow">
            ...
            <CardContent className="p-8 bg-white dark:bg-gray-900">
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                ...
                {/* Audio Upload */}
                <div className="space-y-3">
                  <Label htmlFor="audio" className="text-base font-black uppercase tracking-wide">
                  ...
                  {!audioPreview ? (
                    <div className="border-4 border-dashed border-gray-400 dark:border-gray-600 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 p-12 text-center hover:border-primary hover:bg-gradient-to-br hover:from-primary/5 hover:to-primary/10 transition-all duration-300 group">
                      ...
                      >
                        ...
                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mt-2">
                          Voice will be used for identity verification
                        </p>
                      </label>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      <motion.div
                        initial={{ opacity: 0, scale: 0.8 }}
                        animate={{ opacity: 1, scale: 1 }}
                        className="relative border-2 border-gray-300 dark:border-gray-700 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 p-6 rounded-lg"
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-4">
                            <div className="p-3 gradient-primary shadow-lg">
                              <Volume2 className="h-8 w-8 text-white" />
                            </div>
                            <div>
                              <p className="font-bold text-gray-900 dark:text-white uppercase tracking-wide">
                                {audioFile?.name || "Audio file"}
                              </p>
                              <p className="text-sm text-gray-500 uppercase text-xs tracking-wider">
                          ...
                          <button
                            type="button"
                            onClick={removeAudio}
                            className="bg-red-500 text-white p-2 hover:bg-red-600 transition-colors border-2 border-white rounded-full"
                          >
                            <X className="h-4 w-4" />
                          </button>
                        </div>
